...
function lerp(days: number, d0: number, d1: number, s0: number, s1: number): number {
  if (d1 === d0) return s1;
 ...
 */
export function daysSincePush(f: RepoFacts, now: number = Date.now()): number | null {
  if (!f.pushed_at) return null;
  const t = Date.parse(f.pushed_at);
  if (Number.isNaN(t)) return null;
  return Math.max(0, Math.floor((now - t) / MS_PER_DAY));
}

function bandOf(days: number | null, archived: boolean): Health {
...
function recencyPoints(days: number | null): number {
  ...
  if (days === null) return W_RECENCY * 0.33;
  if (days <= 30) return W_RECENCY;
  if (days <= HEALTHY_DAYS) return lerp(days, 30, HEALTHY_DAYS, W_RECENCY, W_RECENCY * 0.83);
  if (days <= AGING_DAYS) return lerp(days, HEALTHY_DAYS, AGING_DAYS, W_RECENCY * 0.83, W_RECENCY * 0.5);
  if (days <= STALE_2Y_DAYS) return lerp(days, AGING_DAYS, STALE_2Y_DAYS, W_RECENCY * 0.5, W_RECENCY * 0.25);
 ...
 */
function licenseMismatch(license: string | null | undefined, target: string): boolean {
  if (license == null) return false;
  if (isUnlicensed(license)) return false; // NO_LICENSE already covers this
...
export function assess(
  ...
  if (unlicensed) flags.push('NO_LICENSE');
  if (fork) flags.push('FORK');
  if (stale2y) flags.push('STALE_2Y');
  if (f.full_name && f.req.toLowerCase() !== f.full_name.toLowerCase()) flags.push('RENAMED');
  if (licenseMismatch(f.license, target)) flags.push('LICENSE_MISMATCH');
  if ((f.stars ?? 0) < LOW_SIGNAL_STARS && stale) flags.push('LOW_SIGNAL');

  let score = recencyPoints(days) + starPoints(f.stars);
  if (!unlicensed) score += W_LICENSE;
  else score -= P_NO_LICENSE;
  if (!fork) score += W_NOT_FORK;
  if (archived || disabled) score -= P_ARCHIVED;
  ...
  // it third.
  let verdict_fit: 'ok' | 'warn' = 'ok';
  let fit_note: string | undefined;

  if (verdict === 'BORROW') {
    const reasons: string[] = [];
    if (archived) reasons.push('it is archived (read-only, no fixes will land)');
    if (disabled) reasons.push('it has been disabled by GitHub');
    if (unlicensed) {
      reasons.push(
        f.license == null
    ...
    if (stale) {
      reasons.push(
        `its last commit was ${days} days ago${stale2y ? ' (over two years)' : ''}`,
      );
    }
    if (reasons.length > 0) {
      verdict_fit = 'warn';
 ...
 */
export function rank(list: RepoAssessment[]): RepoAssessment[] {
 ...
 */
function rankTier<T extends { r: RepoAssessment; i: number }>(tier: T[]): T[] {
  if (tier.length < 2) return tier;

  const best = Math.max(...tier.map((x) => x.r.health_score));
  const bucketOf = (x: T): number => (x.r.health_score >= best - DEMOTE_GAP ? 0 : 1);
 ...
 */
export function disqualify(a: RepoAssessment): RejectedRepo | null {
  if (!a.exists) {
    return {
      req: a.req,
