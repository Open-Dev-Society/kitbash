...
function rubricSourcePath(): string {
 ...
 */
function extractRefs(source: string): RepoRef[] {
  ...
  for (const m of source.matchAll(re)) {
    ...
    if (owner === 'string' || name === 'string') continue;
...
async function main(): Promise<void> {
  ...
  if (missing.length > 0) {
    console.error('');
    console.error(
      `[check-examples] FAIL: ${missing.length} repo(s) in the rubric do not exist: ${missing.join(', ')}`,
    );
    console.error(
      '  The few-shot example is the text a model imitates most literally. A 404 here',
    );
  ...
  if (unknown.length > 0) {
    console.error('');
    console.error(
      `[check-examples] FAIL: could not verify ${unknown.length} repo(s): ${unknown.join(', ')}`,
    );
...
main().catch((err) => {
  console.error(
    `[check-examples] fatal: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
  );
  process.exitCode = 1;
});
