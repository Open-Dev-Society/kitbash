...
let memo: TokenResolution | null = null;
let memoAt = 0;
...
function fromEnv(name: 'GITHUB_TOKEN' | 'GH_TOKEN'): string | null {
...
function fromGhCli(): string | null {
  try {
    ...
    console.error(
      `[token] \`gh auth token\` unavailable (${code}) — running unauthenticated at 60 req/hr`,
    );
    return null;
  }
}
 ...
 */
export function resolveToken(): TokenResolution {
...
export function __resetTokenMemo(): void {
  memo = null;
  memoAt = 0;
}
