...
export type GhOutcome<T> =
  | { ok: true; data: T; remaining: number | null }
  | { ok: false; kind: GhErrorKind; status: number; retryAfterMs?: number };

/** Last observed x-ratelimit-remaining, across every response this process saw. */
let remainingSeen: number | null = null;

export function lastRemaining(): number | null {
...
export async function ghFetch<T>(
  ...
  if (remaining !== null) remainingSeen = remaining;
