...
/** Only the fields we consume. Field paths verified against real API responses. */
interface GhRepo {
  full_name?: string;
  html_url?: string;
  description?: string | null;
  stargazers_count?: number;
  /** Real commit activity. NEVER updated_at — that bumps on stars/description. */
  pushed_at?: string;
...
function dead(req: string, status: number, fetchedAt: string): RepoFacts {
  return { req, exists: false, status, source: 'live', fetched_at: fetchedAt };
}

function toFacts(req: string, r: GhRepo, fetchedAt: string): RepoFacts {
  return {
    req,
    exists: true,
    status: 200,
...
async function fetchOne(ref: RepoRef): Promise<RepoFacts> {
  const req = reqOf(ref);
  ...
  let outcome;
  try {
    outcome = await ghFetch<GhRepo>(path);
  } catch {
 ...
 */
export async function fetchRepoFacts(refs: RepoRef[]): Promise<RepoFacts[]> {
  if (!Array.isArray(refs) || refs.length === 0) return [];

  const results: RepoFacts[] = new Array(refs.length);
