...
function isRepoFactsLike(v: unknown): v is RepoFacts {
 ...
 */
export function readSnapshot(): Record<string, RepoFacts> {
  let raw: string;
  try {
  ...
  const out: Record<string, RepoFacts> = {};
  try {
    for (const value of Object.values(parsed as Record<string, unknown>)) {
      if (!isRepoFactsLike(value)) continue;
 ...
 */
export function writeSnapshot(facts: RepoFacts[]): void {
  if (!Array.isArray(facts) || facts.length === 0) return;

  try {
    const merged: Record<string, RepoFacts> = {};
    ...
    try {
      const existing = JSON.parse(readFileSync(SNAPSHOT_PATH, 'utf8')) as unknown;
      if (typeof existing === 'object' && existing !== null && !Array.isArray(existing)) {
        for (const [k, v] of Object.entries(existing as Record<string, unknown>)) {
          if (isRepoFactsLike(v)) merged[keyFor(k)] = v;
        }
      }
    } catch {
