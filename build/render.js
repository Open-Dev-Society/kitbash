...
function agoFromDays(days) {
    ...
    if (days < 7)
        return `last commit ${days} days ago`;
    if (days < 14)
        return 'last commit last week';
    if (days < 60)
        return `last commit ${Math.round(days / 7)} weeks ago`;
    if (days < 365)
        return `last commit ${Math.max(2, Math.round(days / 30))} months ago`;
...
function snapshotAge(fetchedAt, now) {
    ...
    if (mins < 60)
        return `${plural(mins, 'minute', 'minutes')} old`;
    const hours = Math.round(mins / 60);
    if (hours < 24)
        return `${plural(hours, 'hour', 'hours')} old`;
    const days = Math.round(hours / 24);
    if (days < 30)
        return `${plural(days, 'day', 'days')} old`;
...
/* repo block                                                          */
/* ------------------------------------------------------------------ */
function statLine(r, now) {
    const bits = [stars(r.stars), agoFromDays(r.days_since_push), license(r.license)];
    if (r.language)
        bits.push(r.language);
    if (r.source === 'cache')
        bits.push(`CACHED SNAPSHOT, ${snapshotAge(r.fetched_at, now)}`);
    return bits.join('  ·  ');
}
function pickedBlock(r, now) {
    const out = [];
    out.push(`**${repoLink(r)}**`);
    ...
    if (r.source === 'cache') {
        out.push('');
        out.push(`_Not live. These numbers are from a snapshot ${snapshotAge(r.fetched_at, now)} — re-verify before you commit to it._`);
    }
    return out;
}
/**
 ...
 */
function demotionReason(demoted, picked) {
    const bits = [];
    if (demoted.archived)
        bits.push('archived');
    if (demoted.disabled)
        bits.push('disabled by GitHub');
    if (demoted.flags.includes('NO_LICENSE'))
        bits.push('no license');
...
/* component block                                                     */
/* ------------------------------------------------------------------ */
function renderComponent(c, now, firstWrite, demoted) {
    const out = [];
    ...
    if (c.verdict === 'WRITE') {
        ...
        if (firstWrite) {
            out.push('');
            out.push('_A component you never take on is a component you never maintain. This is a result, not a gap._');
        }
        return out.join('\n');
    }
    if (c.picked) {
    ...
    if (c.needs_recall) {
        ...
        out.push('>');
        out.push(`> Every repo proposed for this component failed verification${c.rejected.length ? ` (${c.rejected.map(rejectedLabelCode).join(', ')})` : ''}.`);
        out.push('>');
        out.push('> Propose replacement repos for this component and call `kitbash_verify` again.');
    }
    else if (c.rejected.length) {
        out.push('');
        out.push(`_Dropped before you saw it: ${c.rejected.map(rejectedLabel).join(', ')}._`);
    }
    return out.join('\n');
}
/* ------------------------------------------------------------------ */
 ...
 */
export function renderReport(r, now = Date.now(), demotions = new Map()) {
    const out = [];
    ...
    if (comps.length > 0) {
        ...
        out.push(`${plural(comps.length, 'component', 'components')} — ${headline.join(', ')}.`);
        out.push('');
        if (counts.WRITE > 0) {
    ...
    const legendVerdicts = ['BORROW', 'KITBASH', 'WRITE'];
    ...
    const repoCount = comps.reduce((n, c) => n + (c.picked ? 1 : 0) + c.alternates.length, 0);
    ...
    if (r.stats.proposed > 0 && r.stats.verified === 0) {
        out.push(`_GitHub could not be reached — **nothing below was confirmed**. Re-run when the network is back._`);
    }
    else if (repoCount === 0) {
        out.push(`_No repos to check — this slate is all code you write yourself._`);
    }
    else if (r.github.source === 'cache') {
        out.push(`_Served from a cached snapshot — **not live**. Snapshot ages are labelled on each repo below._`);
    }
    else if (r.github.source === 'mixed') {
        out.push(`_Checked against the GitHub API. Some records came from cache and are labelled inline; treat those as unconfirmed._`);
    }
    else {
        const rate = r.github.remaining === null ? '' : ` (${r.github.remaining} API calls left this hour)`;
        out.push(`_Every one of the ${plural(repoCount, 'repo', 'repos')} below returned 200 from the GitHub API just now${rate}._`);
    }
    out.push('');
    /* ---- the demo moment ---- */
    if (r.stats.hallucinated > 0) {
        ...
        out.push('---');
        out.push('');
        out.push(`> ## ${r.stats.hallucinated} of ${r.stats.proposed} proposed repos did not exist and were dropped before you saw them.`);
        out.push('>');
        out.push(`> Hallucination rate: **${pct(r.stats.hallucination_rate)}**. ` +
    ...
    if (r.next_action === 'RECALL_REPLACEMENTS') {
        ...
        out.push('>');
        out.push(`> ${stuck.length === 1 ? 'One component has' : `${stuck.length} components have`} no verified repo: **${stuck.join('**, **')}**.`);
        out.push('>');
