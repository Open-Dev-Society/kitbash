...
const componentSchema = z.object({
    ...
    verdict: z
        ...
        .describe('BORROW = a real maintained dependency exists. KITBASH = good reference, imperfect fit. WRITE = generic enough to just write.'),
    rationale: z
        .string()
 ...
 */
async function fetchWithFallback(refs) {
    if (refs.length === 0)
    ...
    const degraded = indeterminate >= facts.length * DEGRADED_FRACTION || elapsed > SLOW_BATCH_MS;
    let usedCache = false;
    if (degraded && indeterminate > 0) {
        const snapshot = readSnapshot();
        for (let i = 0; i < facts.length; i++) {
            const f = facts[i];
            if (!isIndeterminate(f))
                continue;
        ...
        if (usedCache) {
            console.error(`[verify] degraded batch (${indeterminate}/${facts.length} indeterminate, ${elapsed}ms) — filled from snapshot`);
        }
    }
    ...
    const source = cacheCount === 0 ? 'live' : liveCount === 0 ? 'cache' : 'mixed';
    return { facts, source };
}
function resolveComponent(component, factsByIndex, targetLicense) {
    const survivors = [];
    const rejected = [];
    ...
    const demotedFrom = picked && agentFirst && picked.req !== agentFirst.req ? agentFirst : undefined;
    ...
    const needs_recall = component.verdict !== 'WRITE' &&
 ...
 */
function computeStats(components) {
    let proposed = 0;
    let verified = 0;
    let hallucinated = 0;
    for (const c of components) {
        if (c.verdict === 'WRITE')
            continue;
        const all = [...(c.picked ? [c.picked] : []), ...c.alternates];
...
function buildInstructions(components, stats) {
    ...
    const halluc = stats.hallucinated > 0
    ...
    const writeFloor = noWrite
        ? ' NOTE: this slate has ZERO WRITE components. Every part of this product is a repo you ' +
            ...
            'report as-is.'
        : '';
    return ('DONE. Every component resolved. Present the markdown report above to the user as-is; ' +
...
export function registerVerifyTool(server) {
    server.registerTool('kitbash_verify', {
        ...
        const targetLicense = target_license && target_license.trim() ? target_license.trim() : DEFAULT_TARGET_LICENSE;
        ...
        const resolved = resolutions.map((x) => x.component);
        ...
        for (const x of resolutions) {
            if (x.demotedFrom)
                demotions.set(x.component.id, x.demotedFrom);
        }
        // 6.
        ...
        // 7.
        const next_action = resolved.some((c) => c.needs_recall)
            ? 'RECALL_REPLACEMENTS'
            : 'DONE';
        const report = {
            idea,
        ...
        const result = { ...report, markdown };
        console.error(`[verify] ${resolved.length} component(s), ${stats.proposed} proposed, ` +
